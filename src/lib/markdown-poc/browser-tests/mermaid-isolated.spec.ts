...
test('Mermaid-02 renders only an isolated SVG after hydration and locally falls back for hostile updates', async ({ page }) => {
	const observations = {
	...
	const safeMarkdown = '# Safe Mermaid\n\n```mermaid\nflowchart LR\nA[Start] --> B[End]\n```'
	const hostileSource = 'flowchart LR\nA[<img src=x onerror="window.__markdownPocXss = true">] --> B'
	const hostileMarkdown = `# Safe Mermaid\n\n\`\`\`mermaid\n${hostileSource}\n\`\`\``
	...
	await expect(page.locator('code[data-poc-inert="mermaid"]')).toHaveText(hostileSource)
	...
	await page.locator('.poc-content svg[data-poc-mermaid="rendered"]').evaluate(element => {
	...
	await page.screenshot({ path: 'docs/workflows/markdown-special-adapter-enablement/assets/mermaid-02-final-pass.png', fullPage: true })
