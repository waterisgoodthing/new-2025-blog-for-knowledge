					...
					<div className='space-y-3 rounded-xl border border-orange-200/50 bg-orange-50/30 p-4'>
						...
						<div className='flex gap-3'>
							...
							<input
								...
								list='subject-options-edit'
								placeholder='科目'
								className='flex-1 rounded-lg border border-white/40 bg-white/60 px-3 py-2 text-sm outline-none'
							/>
							<datalist id='subject-options-edit'>
								{subjects.map(s => <option key={s.id} value={s.name} />)}
							</datalist>
						</div>
						<textarea value={form.question} onChange={e => update('question', e.target.value)} placeholder='题目内容...' rows={4} className='w-full rounded-lg border border-white/40 bg-white/60 px-3 py-2 text-sm outline-none' />
						<textarea value={form.my_answer} onChange={e => update('my_answer', e.target.value)} placeholder='我的错误答案...' rows={3} className='w-full rounded-lg border border-red-200/50 bg-red-50/30 px-3 py-2 text-sm outline-none' />
						<textarea value={form.correct_answer} onChange={e => update('correct_answer', e.target.value)} placeholder='正确答案...' rows={3} className='w-full rounded-lg border border-green-200/50 bg-green-50/30 px-3 py-2 text-sm outline-none' />
						<textarea value={form.analysis} onChange={e => update('analysis', e.target.value)} placeholder='分析与反思...' rows={3} className='w-full rounded-lg border border-blue-200/50 bg-blue-50/30 px-3 py-2 text-sm outline-none' />
						<input value={form.knowledge_points} onChange={e => update('knowledge_points', e.target.value)} placeholder='知识点总结' className='w-full rounded-lg border border-purple-200/50 bg-purple-50/30 px-3 py-2 text-sm outline-none' />
					</div>
				) : (
					<>
						<div className='flex gap-1 rounded-lg bg-white/40 p-1'>
							<button onClick={() => setTab('edit')} className={cn('rounded-md px-4 py-1.5 text-sm transition-colors', tab === 'edit' ? 'bg-white shadow-sm font-medium' : 'text-gray-500 hover:text-gray-700')}>编辑</button>
							<button onClick={() => setTab('preview')} className={cn('rounded-md px-4 py-1.5 text-sm transition-colors', tab === 'preview' ? 'bg-white shadow-sm font-medium' : 'text-gray-500 hover:text-gray-700')}>预览</button>
						</div>
