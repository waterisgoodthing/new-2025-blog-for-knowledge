...
export function DraftEditor({ draftId }: { draftId: string }) {
  ...
  useEffect(() => {
    Promise.all([getQuestionDraft(draftId), listSubjects({ is_active: true })])
  ...
  return (
    <div className='max-w-3xl space-y-7'>
      ...
      {draft.item.status === 'converted' && draft.item.target_id ? (
        <Link href={`/manage/questions/${draft.item.target_id}`} className='inline-flex rounded-xl bg-emerald-50 px-4 py-3 text-sm text-emerald-700'>已进入题库，查看正式题目 →</Link>
      ) : null}
      <form onSubmit={save} className='space-y-5 rounded-2xl border border-slate-200/70 p-5'>
        <div className='grid gap-4 sm:grid-cols-2'>
          <label className='text-sm text-slate-600'>科目
            <select disabled={!editable} value={draft.subject_id} onChange={(event) => setDraft({ ...draft, subject_id: Number(event.target.value), knowledge_point_ids: [] })} className='mt-2 w-full rounded-xl border border-slate-200 px-3 py-2.5'>
              {subjects.map((subject) => <option key={subject.id} value={subject.id}>{subject.name}</option>)}
            </select>
          </label>
          <label className='text-sm text-slate-600'>题型
            <select disabled={!editable} value={draft.question_type} onChange={(event) => setDraft({ ...draft, question_type: event.target.value as QuestionType })} className='mt-2 w-full rounded-xl border border-slate-200 px-3 py-2.5'>
        ...
        <label className='block text-sm text-slate-600'>标题（可选）
          <input disabled={!editable} value={draft.title ?? ''} onChange={(event) => setDraft({ ...draft, title: event.target.value || null })} className='mt-2 w-full rounded-xl border border-slate-200 px-3 py-2.5' />
        </label>
        <label className='block text-sm text-slate-600'>题干
          <textarea disabled={!editable} value={draft.question_text} onChange={(event) => setDraft({ ...draft, question_text: event.target.value })} rows={6} className='mt-2 w-full rounded-xl border border-slate-200 px-3 py-2.5' />
        </label>
        {draft.question_type === 'single_choice' || draft.question_type === 'multiple_choice' ? (
          <label className='block text-sm text-slate-600'>选项（每行一项）
            <textarea disabled={!editable} value={optionsText} onChange={(event) => setOptionsText(event.target.value)} rows={5} className='mt-2 w-full rounded-xl border border-slate-200 px-3 py-2.5' />
          </label>
        ) : null}
        <div className='grid gap-4 sm:grid-cols-2'>
          <label className='text-sm text-slate-600'>正确答案
            <textarea disabled={!editable} value={draft.correct_answer ?? ''} onChange={(event) => setDraft({ ...draft, correct_answer: event.target.value || null })} rows={3} className='mt-2 w-full rounded-xl border border-slate-200 px-3 py-2.5' />
          </label>
          <label className='text-sm text-slate-600'>解析
            <textarea disabled={!editable} value={draft.explanation ?? ''} onChange={(event) => setDraft({ ...draft, explanation: event.target.value || null })} rows={3} className='mt-2 w-full rounded-xl border border-slate-200 px-3 py-2.5' />
          </label>
        </div>
        <label className='block text-sm text-slate-600'>难度
          <select disabled={!editable} value={draft.difficulty ?? ''} onChange={(event) => setDraft({ ...draft, difficulty: (event.target.value || null) as Difficulty | null })} className='mt-2 w-full rounded-xl border border-slate-200 px-3 py-2.5'>
        ...
        {editable ? (
          <div className='flex flex-wrap justify-between gap-3'>
            ...
            }} className='inline-flex items-center gap-2 rounded-xl border border-red-200 px-4 py-2 text-sm text-red-600'><XCircle className='h-4 w-4' />拒绝</button>
            <div className='flex gap-3'>
              <button disabled={busy || !draft.question_text.trim()} className='rounded-xl border border-slate-200 px-4 py-2 text-sm'>保存修改</button>
              <button type='button' disabled={busy} onClick={async () => {
                const optionValues = optionsText.split('\n').map((value) => value.trim()).filter(Boolean)
                if ((draft.question_type === 'single_choice' || draft.question_type === 'multiple_choice') && optionValues.length < 2) {
                  setError('选择题至少需要填写两个选项，每行一个。')
              ...
              }} className='inline-flex items-center gap-2 rounded-xl bg-[var(--color-brand)] px-4 py-2 text-sm text-white'><CheckCircle2 className='h-4 w-4' />确认入库</button>
            </div>
          </div>
        ) : null}
      </form>
