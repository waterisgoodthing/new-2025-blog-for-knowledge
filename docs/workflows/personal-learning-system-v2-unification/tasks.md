...
> 当前状态：I0–I10 已完成并通过，`DRY_RUN_READY=PASS`；E-05=`PASS`，
> E-06=`PASS`。授权仅覆盖最小 live 写入、停写、切换、
> 观察、reverse delta、回切和只读归档；不得扩大为应用部署、Git push、
> 无关业务改动或真实 AI 成功调用。
...
- [x] E-05 影子迁移和增量对账（`PASS`；123/123、单 owner、零 orphan、
  幂等 replay、独立 SQL、销毁与 source 不变）
- [x] E-06 权威切换、观察期和旧系统只读归档（`PASS`；最终 authority=
  `blog_v2`，Legacy `blog_db` 与旧附件只读，回切/重切/独立交叉验证完成）

## F. 最终收口

- [x] F-01 完成端到端连接性、恢复、权限、浏览器和代码质量审查
- [x] F-02 完成部署资格审查；技术候选具备单独授权后的部署资格，但当前 dirty
  worktree 被 clean-artifact 门禁阻断，本任务未部署或 push
- [x] F-03 形成最终完成报告，分别声明代码完成、隔离验证、部署技术资格、
  当前工作树可部署性、实际部署和 authority 切换状态

## R1：管理工作区入口与视觉收敛（已执行，PARTIAL）

> 来源：2026-07-30 对 `docs/refactor-plan/` 的重新审查。R1 不继承原方案中的数据迁移、旧路由删除、编辑器统一、认证、存储、部署或生产授权，也不解除 E/F 阶段的任何阻塞。

- [x] R1-00 用户已于 2026-07-30 明确批准 [R1-01 至 R1-10](./r1-workspace-entry-visual-convergence-tasks.md)。
- [x] R1-01～R1-10 已按独立任务清单逐项执行并即时更新状态；实现 diff 与隔离验证通过，但主工作区门禁和失效会话验收未通过，R1 状态为 `PARTIAL / NOT PASS`。
