...
| T11 | Fix CI-exposed 026 defect: ORM lacked `review_items.mistake_id` (model + creation site); CI runs on `backend/.venv` per LSR03 contract | S1 | DONE |
...
- T11: CI runs on the pushed commits exposed that migration 026 lines 1058–1072 deliberately add `review_items.mistake_id UUID NOT NULL` (FK `fk_review_items_mistake` → mistakes ON DELETE RESTRICT, unique index `uq_review_items_mistake_id`, backfilled from `target_id`) while the ORM never wrote it — **every review-items INSERT breaks on any database at revision ≥ 026** (CI: 5 review + 1 dashboard test failures; would break production review scheduling after 026). Static contract tests do not assert the column; the project values model↔migration parity (025's purpose), so the fix honors the migration: `ReviewItem` model gained the column mirroring the exact DDL, and `mistake_service.convert_mistake_draft` sets `mistake_id=m.id` (available post-flush). The earlier local "drift repair" was re-evaluated: the column is 026's intended state, so it was restored locally per the 026 DDL; suite re-run **412 passed / 6 skipped / 0 failed**. `ci.yml` backend job now creates `backend/.venv` and runs everything through `.venv/bin/python`, satisfying the LSR03 runtime contract (`LSR03_PYTHON_RUNTIME_REQUIRED` failures were the harness correctly refusing CI's system interpreter).
