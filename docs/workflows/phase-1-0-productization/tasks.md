...
- [x] **DEP-P0-02** — `PASS`: user approved production registration disabled; Cloudflare invocation logs remain enabled at 1% head sampling with platform short retention. Any higher sampling is separate temporary incident scope. The decision authorizes an implementation task to split frontend/backend gates and set the frontend template policy; it does not authorize target access, backend deployment, migration/DDL, restore or credential read.
...
- [x] **DEP-P1-01A** — `PASS`: test-first split release gates into `predeploy:frontend` (clean worktree, audit, dependency tree, frontend tests/build/typecheck/diff only), `predeploy:backend` (distinct isolated-test/target DB checks), and full composition. Production now rejects `ENABLE_REGISTRATION=true` before database readiness; Workers template sampling is `0.01`. Node gate tests 3/3 and startup-monitoring tests 12/12 pass. Actual frontend gate correctly stops at the dirty-worktree check without database or network/target access. No deployment or production configuration read/write.
- [x] **DEP-P1-01B** — `PASS`: user approved the exact 16-path source/template/test/workflow scope. Commit `929dd08` is the new release artifact; cached scope and whitespace checks passed, unrelated `docs/project-assessment/` and `docs/workflows/mvp-goal-gap-analysis/` remained unstaged, and a temporary detached worktree at this commit was clean before removal. Push follows under the same explicit instruction; no deployment under this item.
- [x] **DEP-P1-02** — `PASS / FRONTEND ONLY`: after user authorized safe presence/validity reading of the non-secret build variable, the detached clean worktree at `cc05ef5` passed frontend gate: 43/43 tests, TypeScript, Cloudflare build and diff hygiene. Backend test/target DB check was not requested or run.
- [x] **DEP-P1-03** — `PASS / FRONTEND ONLY`: Cloudflare Worker `2025-blog-public` deployed as version `d79488b8-705b-48a1-b562-ab2fcf872589` at 100%. No backend deployment, target DB access, migration, DDL, restore or attachment-storage action occurred.
- [ ] **DEP-P1-04** — `PARTIAL / PASSWORD-ROTATION CONFIRMATION PENDING`: the initial `workers.dev` failure is now classified as an unsupported-origin CORS boundary, not a public API outage. Fresh anonymous browsers against the formal frontend domain passed public homepage and `/manage`: document, first-party JS/CSS, hydration/navigation, console (0 errors), public API preflight/reads and expected anonymous `auth/me` 401. `/manage` runtime requests its passkey status from `public-api`, disproving the earlier static-chunk-only relative-path concern. Screenshots and hashes are in validation. A user-held production Passkey session then passed read-only Dashboard, attachment workspace and settings smoke with 0 console errors; the security settings UI confirms a high-privilege Passkey session and provides a password-rotation form. Public worker endpoints are not a supported browser origin. Password rotation, administrator write and diagnostics detail remain pending explicit action-time confirmation; no backend target deployment, migration, DDL, restore or storage action occurred.
...
## Deployment Repair — Production Password Rotation

> PLANNED / AWAITING APPROVAL: root cause is confirmed from a real Passkey session: `src/app/manage/security-tab.tsx` bypasses the configured API client and posts to the relative frontend path `/api/auth/set-password`, which has no formal-domain API proxy. The following minimal `auth`/frontend-contract task does not authorize password submission, credential capture, production data changes, migration/DDL, backend deployment or unrelated refactoring.

- [ ] **DEP-P1-04A** Add a failing focused test for the security-tab API-base contract; replace only the relative password-update fetch with a typed configured-base auth API wrapper; run the focused test, TypeScript and production build. Update this task immediately after completion. Real password submission remains a separate user-confirmed browser action after the frontend release succeeds.
