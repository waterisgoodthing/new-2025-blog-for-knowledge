...
## 2026-07-19 DEP-P1-02 / DEP-P1-03 Frontend Release

After a user-authorized safe check confirmed only that the production API base was present and non-local, the frontend-only gate passed in the isolated `cc05ef5` worktree: frontend tests 43/43, TypeScript, Cloudflare build and diff hygiene. Cloudflare Worker `2025-blog-public` deployed version `d79488b8-705b-48a1-b562-ab2fcf872589` at 100%. The backend boundary was intentionally untouched: no target database, Alembic, backend deploy, migration, DDL, restore or attachment-storage operation occurred.

## 2026-07-19 DEP-P1-04 Public Smoke Boundary Resolution

The public Worker document rendered in a fresh anonymous browser. The first constrained browser session produced an allowlist-induced API block and was discarded as invalid evidence. A second `workers.dev` session exposed the actual boundary: a direct safe probe later showed `Origin: workers.dev` preflight returns 400, while the public API health endpoint is 200. The Worker URL is therefore not a supported browser origin, rather than evidence of a public API outage. A fresh anonymous session against the formal frontend domain `blog.limengyang.me` proved document and first-party JS/CSS 200, public API preflight and reads 200, successful client navigation/hydration, and 0 console errors. The expected anonymous `auth/me` 401 did not affect UI or console state. Evidence is `assets/2026-07-19-public-formal-home.png` (hash recorded in validation). No admin credentials, backend target access, deployment, migration, DDL, restore or attachment-storage operation occurred.

A separate fresh anonymous `/manage` session verified the operational login surface without supplying credentials: document, JS and CSS loaded from the formal frontend domain; site settings, daily-song public data, anonymous `auth/me`, and passkey status all requested the configured `public-api` origin. The expected anonymous `auth/me` 401 did not create a console error or render failure. This supersedes the static-chunk-only hypothesis that management auth might use the relative frontend `/api` path. The screenshot is `assets/2026-07-19-public-formal-manage-anon.png`; administrator actions remain intentionally untested until a secret-safe real production session exists.

The user then supplied a real production Passkey session in Chrome. Read-only browser checks reached the protected Dashboard, attachment workspace and settings surfaces with no visible alert or console error. The attachment workspace showed the protected upload/list UI and an existing private attachment record without opening its content. The security surface confirmed the session is Passkey high privilege and exposes a password-rotation form. The password itself was neither generated, read, transmitted nor recorded; final submission remains an explicit action-time confirmation. This validates the protected routing and storage-list connectivity without creating, modifying or deleting production records.

The user-reported password-update failure was investigated before any retry. The real security page retained no safe UI error or console error after the submission, so the source/data-flow boundary was inspected. `src/app/manage/security-tab.tsx` posts directly to the relative frontend URL `/api/auth/set-password`; the established public and management API client instead derives the configured `public-api` base. Because the formal frontend does not proxy that relative backend route, the failure is a deterministic frontend-origin routing defect, not an invalid Passkey session or backend password contract failure. Password input was not copied into workflow evidence.
...
Frontend-only validation was run from an isolated clean worktree at `cc05ef5`. Dependency installation completed with the known Node-engine warning and three moderate audit findings; frontend tests passed 43/43. The Cloudflare build then failed deterministically because the production API client requires a non-local `NEXT_PUBLIC_API_URL`, while the clean release artifact intentionally contains no environment file. The source's example value is localhost and existing build-only evidence uses a deliberately invalid host, neither of which is valid for public deployment. No target configuration was read, no endpoint was guessed, and no deploy command was run. The isolated worktree is retained for an authorized retry.
